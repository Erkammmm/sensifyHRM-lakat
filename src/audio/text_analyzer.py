...
def _sanitize_stt_model_name(name: str) -> str:
    n = (name or "").strip()
    # Env yanlış set edilirse burada güvenli default'a dön.
...
class TextAnalyzer:
    ...
    def _ensure_sentiment(self) -> None:
        ...
        logger.info(
            "[%s] Sentiment modeli yüklendi. Device=%s",
    ...
    def _ensure_fw_model(self) -> None:
        ...
        def build_candidates(name: str) -> List[str]:
            ...
            for c in cands:
                ...
                if "/" in c:
                    owner, repo = c.split("/", 1)
                    if not is_valid_repo_id(owner) or not is_valid_repo_id(repo):
                        continue
        ...
        if device == "cuda":
            for ct in ["float16", "int8_float16", "int8"]:
                if ct not in compute_fallbacks:
                    compute_fallbacks.append(ct)
        ...
        for cand in candidates:
            for ct in compute_fallbacks:
                try:
                    ...
                    logger.debug(
                        "[%s] faster-whisper (%s) yüklendi -> %s (%s)",
                        self.__class__.__name__,
                        cand,
                        device.upper(),
                        ct,
        ...
        if torch.cuda.is_available() and device == "cuda":
            try:
                ...
                logger.info(
                    "[%s] faster-whisper CPU fallback yüklendi -> CPU (int8)",
                    self.__class__.__name__,
                )
                return
            except Exception as exc:
    ...
    def _ensure_hf_asr(self, force_cpu: bool = False) -> None:
        """
        Transformers ASR pipeline ile Whisper STT yükler.
        ...
        logger.debug(
            "[%s] transformers ASR (%s) yüklendi -> %s",
            self.__class__.__name__,
            model_id,
            "CUDA" if device_idx == 0 else "CPU",
        )
    ...
    def process_video(
        ...
        progress_callback:
          0.0 - 1.0 arası ilerleme bildirimi yapar.
        """
        ...
        long_audio_cpu_threshold = float(os.getenv("SENSIFYHR_STT_LONG_AUDIO_CPU_SEC", "900"))
        ...
        # 1) Opsiyonel: transformers ASR
        if use_transformers:
            try:
                logger.debug("Dosya işleniyor (transformers ASR): %s", video_path)
                ...
                total_seconds = duration_sec or 0.0
                ...
                for ch in chunks:
                    ...
                    if progress_callback and total_seconds > 0:
                        progress_callback(min(1.0, end_time / total_seconds))
                ...
                if segments_data:
                    if torch.cuda.is_available():
                        torch.cuda.empty_cache()
                    logger.debug("Metin analizi tamamlandı: %s segment.", len(segments_data))
                ...
                if _is_cuda_oom(exc) and torch.cuda.is_available():
                    try:
                        torch.cuda.empty_cache()
                        self._hf_asr_pipe = None
                        logger.warning("[TextAnalyzer] CUDA OOM -> transformers ASR CPU fallback deneniyor...")
        ...
        logger.debug("Dosya işleniyor (faster-whisper fallback): %s", video_path)
        ...
        try:
            logger.debug("[TextAnalyzer] Transcribe çağrısı hazırlanıyor (VAD filter=True, beam_size=1)...")
            ...
            logger.debug(
                "[TextAnalyzer] Transcribe çağrıldı (dil: %s). İlk segment bekleniyor...",
                info.language,
            )
        except Exception as exc:
            if _is_cuda_oom(exc) and torch.cuda.is_available():
                ...
                logger.warning("[TextAnalyzer] CUDA OOM -> faster-whisper CPU(int8) retry...")
                model_name = _sanitize_stt_model_name(STT_MODEL)
        ...
        total_seconds = duration_sec or _get_audio_duration_seconds(video_path)

        for seg in segments:
            start_time = float(getattr(seg, "start", 0.0) or 0.0)
            end_time = float(getattr(seg, "end", start_time) or start_time)
            text = " ".join((getattr(seg, "text", "") or "").strip().split())
