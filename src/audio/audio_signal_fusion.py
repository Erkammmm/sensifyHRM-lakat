...
# Gerçek veriden kalibre edildi
VOICE_PROFILES: Dict[str, Dict[str, float]] = {
    "Canlı": {"valence": +0.6, "arousal": +0.7},
    "Kararlı": {"valence": +0.3, "arousal": +0.5},
    "Gergin": {"valence": -0.4, "arousal": +0.6},
    "Dengeli": {"valence": +0.1, "arousal": +0.2},
    "Sakin": {"valence": -0.1, "arousal": -0.2},
}
...
def _estimate_onsets_per_second(audio: np.ndarray, sr: int) -> float:
    ...
    rms = torch.sqrt(torch.mean(frames**2, dim=1) + _EPS)
    ...
    delta = max(delta, float(np.max(onset_env)) * 0.10)
...
class AudioSignalFusion:
    ...
    def __init__(self):
        logger.debug("[AudioSignalFusion] Başlatılıyor (f0+enerji kural sistemi)...")
        self.device = "cpu"
        logger.debug("[AudioSignalFusion] Hazır!")

    def process_audio(self, audio_path: str) -> List[Dict]:
        if not os.path.exists(audio_path):
            logger.warning("[AudioSignalFusion] Audio bulunamadı: %s", audio_path)
            return []
        ...
        logger.debug("[AudioSignalFusion] Audio signal tamamlandı: %s parça.", len(timeline))
        return timeline
